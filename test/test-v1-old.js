...
async function runTests() {
  ...
  // Test 3: Message Encryption and Decryption - Ephemeral Mode
  ...
  // Test 4: Private Chat Encryption (Authenticated Mode)
  await test('Private Chat Encryption (Authenticated Mode)', async () => {
  ...
  await test('Cross-compatibility: Sign with one key, encrypt with another', async () => {
    ...
    const encrypted = await encryptMessageWithMeta(message, bob.epub);
  ...
  await test('Handle Edge Cases', async () => {
    ...
    const unicodeEncrypted = await encryptMessageWithMeta(unicodeMessage, keys.epub);
